...
export const startGrpcServer = async () => {
  ...
  server.addService(productsPackage.Products.service, {
    getProduct: grpcHandler<ProductRequest, Partial<CurrentProducts> | null>(
      observer.getProduct,
    ),
    getProducts: grpcHandler<
      ProductsRequest,
      { items: Partial<CurrentProducts>[] }
    >(observer.getProducts),
  });
  server.addService(productsPackage.AdminProduct.service, {
    ...
    archiveProduct: grpcHandler<ProductRequest, achiveResponseI>(
      observer.archiveProduct,
    ),
    unarchiveProduct: grpcHandler<ProductRequest, achiveResponseI>(
      observer.unarchiveProduct,
    ),
    createCategory: grpcHandler<CategoriesI, CategoryResI>(
